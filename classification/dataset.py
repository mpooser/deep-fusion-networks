...
#-------------------------------- import split lists--------------------------------
        if label_type == "multi_label" or label_type == "single_label":
            ...
            self.samples = []
            if subset == "train":
                pbar = tqdm(total=108912)  # sapelo2 var
                # pbar = tqdm(total=10000)
            if subset == "val":
                pbar = tqdm(total=53643)   # sapelo2 var
            if subset == "test":
...
# new subclass for SEN12MS
class SEN12MS2I(SEN12MS):
    ...
    def __init__(self, path, ls_dir=None, imgTransform=None,
        ...
        super(SEN12MS2I, self).__init__(path, ls_dir, imgTransform,
                     label_type, threshold, subset, use_s2, use_s1, use_RGB, IGBP_s)

        # override number of input channels to reflect a tuple/list
        self.n_inputs = (get_ninputs(use_s1=True, use_s2=False, use_RGB=use_RGB), get_ninputs(use_s1=False, use_s2=True, use_RGB=use_RGB))

    def __getitem__(self, index):
        # returns 2 items from the dataset

        # get and load sample from index file
        sample = self.samples[index]
        labels = self.labels
        a = load_sample(sample, labels, self.label_type, self.threshold, self.imgTransform,
                           True, False, self.use_RGB, self.IGBP_s)
        b = load_sample(sample, labels, self.label_type, self.threshold, self.imgTransform,
                           False, True, self.use_RGB, self.IGBP_s)
        return (a, b)
...
if __name__ == "__main__":
    ...
    # data path
    data_dir = "../"    # SEN12MS dir
    list_dir = "../labels_splits"    # split lists/ label dirs
    ...
    print("\n\nSEN12MS train")
    #ds = SEN12MS2I(data_dir, list_dir, imgTransform,
    #             label_type="multi_label", threshold=0.1, subset="train",
    #             use_s1=True, use_s2=True, use_RGB=True, IGBP_s=True)
    ds = SEN12MS2I(data_dir, list_dir, imgTransform,
                 label_type="multi_label", threshold=0.1, subset="train",
                 use_s1=True, use_s2=True, use_RGB=False, IGBP_s=True)

    ds2 = SEN12MS(data_dir, list_dir, imgTransform,
                 label_type="multi_label", threshold=0.1, subset="train",
                 use_s1=True, use_s2=True, use_RGB=False, IGBP_s=True)

    (samp_a, samp_b) = ds.__getitem__(50)
    samp = ds2.__getitem__(50)
    print(f"ds1> num inputs: {ds.n_inputs}")
    print(f"ds1> num classes: {ds.n_classes}")
    print(f"ds2> num inputs: {ds2.n_inputs}")
    print(f"ds2> num classes: {ds2.n_classes}")

    #print(f"numSample a: {samp_a["image"].size(0)}")
    #print(f"numSample b: {samp_b["image"].size(0)}")
    print("-"*80)
    #print(f"numSample: {samp["image"].size(0)}")
