...
def avglogits(x : torch.Tensor, y : torch.Tensor, w_x = 1, w_y = 1) -> torch.Tensor:
    return (x*w_x + y*w_y)/(w_x + w_y)

if __name__ == "__main__":

    inputs = torch.randn((100, 2, 256, 256))
    model_s1 = ResNet101(n_inputs = 2)
    s1_logits = model_s1(inputs)
    print(f"sentinel1 logit size: {s1_logits.shape}")

    inputs = torch.randn((100, 10, 256, 256))
    model_s2 = ResNet101(n_inputs = 10)
    s2_logits = model_s2(inputs)
    print(f"sentinel2 logit size: {s2_logits.shape}")

    inputs = torch.randn((100, 12, 256, 256))
    net = ResNet101()
    outputs = net(inputs)
    print(outputs)
    print(f"sentinel1+2 logit size: {outputs.shape}")

    print(30*"*")
    z = avglogits(s1_logits, s2_logits, w_x = 3, w_y = 1)
    print(z)
    print(z.shape)
